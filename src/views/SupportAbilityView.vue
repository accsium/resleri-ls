  ...
  >
    ...
    <template #cell-avatar="{ row }">
      <div style="text-align:center;line-height:0">
        <AvatarDisplay v-if="charIndexMap[row.id]" :indexEntry="charIndexMap[row.id]" :scale="2" :size="0" />
      </div>
    </template>
    ...
    <template #cell-maxRarity="{ row }">
      <StarsDisplay :rarity="row.max_rarity" :size="0" />
    </template>
    <template #cell-saAttr="{ row }">
      <IconDisplay v-if="row.support_ability?.attr" type="attribute" :id="row.support_ability.attr" :scale="1" :size="0" />
    </template>
    <template #cell-saRole="{ row }">
      <IconDisplay v-if="row.support_ability?.role" type="role" :id="row.support_ability.role" :scale="1" :size="1" />
    </template>
